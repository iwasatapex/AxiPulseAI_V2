...
def attach_probabilistic_analysis(
    ...
    """Canonical NPS categorical probabilistic attachment."""
    import copy
    ...
    # Bayesian update is delegated to the canonical categorical operation.
    bayesian = (
        from_observed_counts(
            ml_distribution, observed_counts, prior_strength=10.0,
            metadata={"source": "nps_model"},
        )
        if observed_counts is not None
        else from_prior_only(
            ml_distribution, prior_strength=10.0,
            metadata={"source": "nps_model"},
        )
    )
    posterior = _extract_posterior(bayesian)

    # Monte Carlo - exactly one canonical categorical simulation.
    ...
    # Preserve the deterministic ML NPS/bucket result. Probabilistic analysis
    # is additive evidence and must not replace the model's point estimate.
    result["monte_carlo_nps"] = {
