...
def simulate(
    ...
    ``bounds`` optionally clips the realized draw to a ``(low, high)`` domain
    (e.g. ``(0.0, 1.0)`` for a probability-domain simulation). The clip is
    applied to the SAME single draw, before any summary is computed, so the
    percentiles, bins and counts all remain consistent with that one execution
    (no second simulation is run). The generic engine stays unbounded by
    default; only probability-domain callers pass bounds.
    ...
    if bounds is not None:
        if len(bounds) != 2:
            raise ValueError("bounds must be a (low, high) interval")
    ...
    return _summarize(
        ...
        seed=seed,
        distribution=distribution,
        metadata={"bounds": [low, high]} if low is not None else None,
    )
