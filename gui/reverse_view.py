...
def render() -> None:
    c.page_title("Reverse Optimizer", eyebrow="Optimization", help_text="Reverse-optimise KPIs for a target")
    ...
    st.caption(
        "Pick a metric and a target value; the engine finds the operational KPIs "
        "that get you closest to it. Delegates to the canonical **TargetStateEngine** "
        "— expect a minute or two per run."
    )

    c.section("Objective", "\U0001f3af")
    ...
    if not result:
        st.write("")
        c.empty_state("Set an objective above and run the optimizer to see recommended KPIs.",
                      icon="\U0001f501")
        return

    st.divider()
    c.section(f"Recommended KPIs for {metric} = {result.get('target')}", "\u2705")
    ...
    with m1:
        c.kpi_tile(f"Predicted {metric}", f"{predicted:.2f}" if predicted is not None else "\u2014",
                    status="ready" if predicted is not None else "none")
    with m2:
        c.kpi_tile("Distance from target", f"{distance:.3f}" if distance is not None else "\u2014",
                    status="none")
    with m3:
        c.kpi_tile("Target", f"{result.get('target')}", status="none")
    ...
    with st.expander("Consensus predictions", expanded=False):
        st.json(result.get("consensus") or {})
