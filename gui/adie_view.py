...
def render() -> None:
    c.page_title("ADIE Decision", eyebrow="Decision Intelligence", help_text="Canonical Decision Intelligence V3 output")
    ...
    with st.form("adie_form"):
        c.section("Decision inputs", "\U0001f39b\ufe0f")
        c1, c2 = st.columns(2)
    ...
    if not result:
        c.empty_state("Run an ADIE decision to see canonical V3 output.", icon="\U0001f9e0")
        return
    ...
    # Detail sections from enriched detail — progressive disclosure below
    # the executive decision, which is the primary surface.
    if details:
        st.divider()
        c.section("Supporting evidence", "\U0001f50e")

        with st.expander("Forecast overview & per-day table", expanded=False):
        ...
        with st.expander("Bayesian analysis", expanded=False):
            _render_bayesian_detail(details.get("bayesian_detail", {}))

        with st.expander("Monte Carlo analysis", expanded=False):
        ...
        with st.expander("Agreement & conflicts", expanded=False):
    ...
    # Forecast context
    st.divider()
    c.section("Forecast context", "\U0001f4c8")
...
def _render_executive_decision(package: dict, recommendations: list[dict] | None = None) -> None:
    ...
    """
    c.section("Executive decision", "\u2696\ufe0f")
    ...
    risk_status = {"LOW": "ready", "MEDIUM": "degraded", "HIGH": "error"}.get(risk, "none")

    cols = st.columns(4)
    with cols[0]:
        c.kpi_tile("Recommendation", rec_label, status="ready" if recs else "none", help_text=rec_help)
    with cols[1]:
        c.kpi_tile("Risk Level", str(risk), status=risk_status)
    with cols[2]:
        prob = package.get("probability")
        c.kpi_tile("Probability", f"{prob:.3f}" if isinstance(prob, (int, float)) else "\u2014", status="none")
    with cols[3]:
        conf = package.get("confidence")
        c.kpi_tile("Confidence", f"{conf:.3f}" if isinstance(conf, (int, float)) else "\u2014", status="none")

    st.write("")
    cols2 = st.columns(4)
    with cols2[0]:
        exp = package.get("expected")
        c.kpi_tile("Expected", f"{exp:.3f}" if isinstance(exp, (int, float)) else "\u2014", status="none")
    with cols2[1]:
        down = package.get("downside")
        c.kpi_tile("Downside (p05)", f"{down:.3f}" if isinstance(down, (int, float)) else "\u2014", status="none")
    with cols2[2]:
        up = package.get("upside")
        c.kpi_tile("Upside (p95)", f"{up:.3f}" if isinstance(up, (int, float)) else "\u2014", status="none")
    with cols2[3]:
        risk_score = package.get("risk_score")
        c.kpi_tile("Risk Score", f"{risk_score:.3f}" if isinstance(risk_score, (int, float)) else "\u2014",
                    status=risk_status)
...
def _render_forecast_overview(fc_summary: dict) -> None:
    ...
    """
    c.section("Forecast overview", "\U0001f52d")
    if not fc_summary:
...
def _render_forecast_table(per_day: list[dict]) -> None:
    ...
    (True/False), never NULL when present.
    """
    c.section("Forecast table (per day)", "\U0001f4c5")
    if not per_day:
    ...
    view = df[display_cols] if display_cols else df
    ...
    st.caption("`_predicted=True` → forecast-generated day; `_predicted=False` → observed day.")


def _render_bayesian_detail(bayesian: dict) -> None:
    """Render Bayesian posterior detail."""
    c.section("Bayesian analysis", "\U0001f4d0")
    if not bayesian:
    ...
    cols = st.columns(2)
    with cols[0]:
        st.metric("Decision Probability", _fmt(bayesian.get('decision_probability')))
        st.metric("Posterior Mean", _fmt(bayesian.get('posterior_ranges', {}).get('mean')))
    with cols[1]:
        st.metric("Confidence", _fmt(bayesian.get('confidence')))
    ...
    st.metric("Credible Interval", (f"[{_fmt(cred.get('lower'))}, {_fmt(cred.get('upper'))}] "
                  f"({_fmt((cred.get('level')*100) if isinstance(cred.get('level'), (int, float)) else None, '.0f')}%)"))
...
def _render_mc_detail(mc: dict) -> None:
    ...
    Two distinct statistics, rendered separately so they are never conflated:

      1. Binary target-attainment outcome: success_count / failure_count /
         success_percentage / failure_percentage = P(OH >= target_oh).
      2. Continuous Monte Carlo score: expected_value / p05 / p50 / p95 /
         uncertainty = the underlying continuous OH distribution (NOT a binary
         0/1 expectation and never presented as the expected binary success).

    Success rate is shown only when a target exists; otherwise it is
    explicitly "Unavailable", never an invented "sample > 0" figure.
    """
    c.section("Monte Carlo analysis", "\U0001f3b2")
    if not mc:
...
def _render_scenario_comparison(scenarios: list[dict]) -> None:
    ...
    c.section("Scenario comparison", "\U0001f501")
    if not scenarios:
...
def _render_sensitivity_detail(sens: dict) -> None:
    ...
    c.section("Sensitivity analysis", "\U0001f39a\ufe0f")
    if not sens:
...
def _render_trend_detail(trend: dict) -> None:
    """Render trend detail."""
    c.section("Trend analysis", "\U0001f4c8")
    if not trend:
...
def _render_agreement_detail(agreement: dict) -> None:
    ...
    c.section("Agreement & conflicts", "\U0001f91d")
    if not agreement:
...
def _render_recommendations(recs: list[dict]) -> None:
    ...
    c.section("Top recommendations", "\U0001f4a1")
    if not recs:
    ...
    for r in recs[:3]:
        with st.expander(f"#{r.get('rank', '?')} — {r.get('action', 'Action')}", expanded=True):
            ...
            with cols[1]:
                st.metric("Confidence", _fmt(r.get('confidence'), '.2f'))
...
def _render_explanation(explanation: dict) -> None:
    """Render enhanced explanation as human-readable prose.

    Raw structured JSON is exposed only inside the collapsed Technical details
    expander, never as a primary st.json dump.
    """
    c.section("Decision explanation", "\U0001f4dd")
    if not explanation:
        st.caption("No explanation available.")
        return

    decision_status = explanation.get("decision_status")
    insufficient = decision_status == "insufficient_evidence" or (
        (explanation.get("recommended_action") or {}).get("action") == "withheld"
    )
    ...
    fp = explanation.get("forecast_preference", {})
    ws = explanation.get("why_selected", {})
    if insufficient:
        name = fp.get("name") or ps.get("name")
        st.write(f"Forecast preference: **{_format_scenario_name(str(name))}** (non-actionable).")
        if ws.get("text"):
            st.caption(ws["text"])
    else:
        if ps:
            st.write(f"Preferred: **{_format_scenario_name(str(ps.get('name')))}** "
                     f"(rank {ps.get('rank')}, score {_fmt(ps.get('score'))})")
        wp = explanation.get("why_preferred", {})
        if wp and wp.get("policy"):
            st.caption(wp["policy"])

    st.markdown("**Main Risk**")
    mr = explanation.get("main_risk", {})
    if insufficient:
        st.write("**Risk decision: ABSTAIN**")
        st.caption(mr.get("reason") or "Canonical decision withheld: insufficient evidence")
    else:
        st.write(f"Risk level: **{mr.get('level', '—')}**")
    ...
    # Raw structured package only in a collapsed Technical details expander.
...
def _format_current_state(cs: dict) -> str:
    ...
    if observed:
        labels = [_label_metric(str(m)) for m in observed if m]
        if labels:
...
def _format_forecast_outlook(fo: dict) -> str:
    ...
    parts = []
    parts.append(f"Scenario count: {fo.get('scenario_count', 0)}")
...
def _format_uncertainty(unc: dict) -> str:
    """Format uncertainty as human-readable text."""
    if not unc:
        return "No uncertainty information available"
    parts = []
    if _fnum(unc.get("downside")) is not None:
        parts.append(f"Downside (p05): {_format_percent(unc.get('downside'))}")
    if _fnum(unc.get("upside")) is not None:
        parts.append(f"Upside (p95): {_format_percent(unc.get('upside'))}")
    if _fnum(unc.get("confidence")) is not None:
        parts.append(f"Confidence: {_format_percent(unc.get('confidence'))}")
    samples = unc.get("monte_carlo_samples")
    if samples is not None:
        parts.append(f"Monte Carlo samples: {samples:,}")
    if unc.get("interpretation"):
        parts.append(str(unc["interpretation"]))
...
def _format_scenario_name(name: str) -> str:
    ...
    if "forecast_day_" in name:
        return f"Forecast Day {name.split('_')[-1]}"
...
def _label_metric(key: str) -> str:
    """Convert an internal identifier to a human-readable label."""
    if not key:
        return ""
    labels = {
...
def _render_technical_details_expander(details: dict, title: str = "Technical details") -> None:
    """Add a collapsed Streamlit expander with raw JSON for debugging."""
