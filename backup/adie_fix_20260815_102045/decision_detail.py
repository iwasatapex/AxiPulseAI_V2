...
def _build_mc_detail(monte_carlo_detail: dict[str, Any]) -> dict[str, Any]:
    ...
    if not (_finite(success_count) and _finite(failure_count)):
        success_count = None
        failure_count = None
    success_pct = None
    failure_pct = None
    if success_count is not None and failure_count is not None:
        total = float(success_count) + float(failure_count)
        if total > 0:
            success_pct = round(float(success_count) / total * 100, 2)
            failure_pct = round(float(failure_count) / total * 100, 2)

    return {
        "total_samples": _round(distribution_summary.get("samples"), 0),
        "success_count": success_count,
        "failure_count": failure_count,
        "success_percentage": success_pct,
        "failure_percentage": failure_pct,
        "expected_value": _round(distribution_summary.get("mean")),
        "p05": _round(distribution_summary.get("p05")),
        "p50": _round(distribution_summary.get("p50")),
        "p95": _round(distribution_summary.get("p95")),
