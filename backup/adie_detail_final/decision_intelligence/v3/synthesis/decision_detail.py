        ...
        if isinstance(data, list):
            for r in data[:3]:
                ...
                recs.append({
                    ...
                    "confidence": _round(r.get("confidence", 0.5)),
                    "risk": "MEDIUM" if r.get("confidence", 0.5) < 0.7 else "LOW",
            ...
            if target_nps is not None and nps is not None:
                if _finite(nps) and _finite(target_nps):
                    recs.append({
                        ...
                        "confidence": _round(best.get("confidence", 0.5)),
                        "risk": _round(risk) if (risk := _assess_risk(best.get("probability", 0.5), best.get("confidence", 0.5))) else "MEDIUM",
...
def _build_forecast_summary(
    ...
    for i, s in enumerate(scenarios):
        ...
        if "nps_p05" in s:
            day_entry["nps_p05"] = s["nps_p05"]
            day_entry["nps_p95"] = s.get("nps_p95")
...
def _build_risk_detail(
    ...
    """Build risk detail from package."""
    return {
        ...
        "thresholds": {
            "high_risk": {"probability": "<", "confidence": "<"},
            "medium_risk": {"probability": ">= 0.5", "confidence": ">= 0.5"},
        },
    }
